...
- BET、Langmuir、t-Plot 结果表的“文件名”表头右侧支持行列切换，三表共用并记忆布局；转置后左侧冻结参数，顶部显示样品与状态标记，单击参数名排序，拖动仍可选取单元格复制。
- 下方七个详情标签不显示表格悬浮提示；点击表格外部或空白处清除复制选区，右键复制保留选区。表头单击仅排序、不整列选中；文件名联动仅加粗，蓝色底色只用于实际选中的单元格。
- “选区孔容量”表并列显示每个样品的 BJH、DH、HK、DFT 绿色选区孔容，支持排序联动与拖选复制；BJH、DH 默认范围相同，调整和恢复默认互不影响。样品栏原有选区孔容列保留。
- 结果表、实际等温线与目标压力表支持拖选单元格、右键复制与 Ctrl+C，便于粘贴到 Excel。
